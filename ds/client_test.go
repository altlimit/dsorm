...
func TestClient_onError(t *testing.T) {
	...
	c, err := ds.NewClient(ctx, testCache, ds.WithDatastoreClient(dsClient))
	if err != nil {
		t.Fatalf("could not make dsorm client due to error: %v", err)
	}
	...
	c, err = ds.NewClient(ctx, testCache, ds.WithDatastoreClient(dsClient), ds.WithOnErrorFunc(onErrFn))
	if err != nil {
		t.Fatalf("could not make dsorm client due to error: %v", err)
	}
...
func TestNewClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := ds.NewClient(tt.args.ctx, tt.args.cacher)
